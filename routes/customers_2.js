        ...
        req.getConnection(function(err,connection){
            connection.query('SELECT * FROM customer WHERE userid = ?',[userid],function(err,profiledata)
            {
                if(err)
                    ...
                    getPrivateMessage(req,userid, function(messageData) {
                        console.log(messageData);
                        res.render('profile',{page_title:"My Profile",data:profiledata,globalMessage: messages,usersmessage: messageData});
                    });
                }
            });
        });
...
function getPrivateMessage(req,userid,callback){
    var displayUserName = '';
    var displaymessage = '';
    var displayFullmessage = '';
    var usermessage = [];
    req.getConnection(function(err,connection){
        connection.query(' SELECT * FROM `conversation` WHERE `sender` = ? or `receiver` = ? ',[userid,userid],function(err,rows)
        {
            if(err)
                console.log("Error Selecting : %s ",err );
            else{
                if(rows.length > 0){
                    for(var i = 0; i < rows.length; i++){
                        ...
                        if(sender === userid){
                            var otheruser = receiver;
                        } else {
                            var otheruser = sender;
                        }
                        connection.query(' SELECT username FROM `users` WHERE `id` = ? ',[otheruser],function(err,usernamrResult)
                        {
                            if(err)
                                console.log("Error Selecting : %s ",err );
                            else {
                                if( usernamrResult.length > 0 ){
                                    var otherusername = usernamrResult[0].username;
                                }
                            }
                            displayUserName += "<div id='user-"+otheruser+"'>"+otherusername+"</div>";
                            
                            connection.query(' SELECT * FROM `privatemessage` WHERE (`senderuserid` = ? AND `reciveruserid` = ?) or (`senderuserid` = ? AND `reciveruserid` = ?) ORDER BY `messagetime` ASC ',[otheruser,userid,userid,otheruser],function(err,messageResult)
                            {
                                if(err)
                                    console.log("Error Selecting : %s ",err );
                                else {
                                    if(messageResult.length > 0){
                                        for(var j= 0; j < messageResult.length; j++) {
                                            displaymessage += "<div>"+messageResult[j].message+"</div>";
                                        }
                                    }
                                    displayFullmessage += "<div id='user-"+otheruser+"'>"+displaymessage+"</div>";
                                    usermessage[i] = {
